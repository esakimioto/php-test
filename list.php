...
$wheres = [];
if (!empty($namae1)) {
 $wheres[] = "(namae1 LIKE '%{$namae1}%' OR  LIKE ' %{$namae1}%')";
}
...
if ($mysqli->connect_error) {
    ...
    // 接続成功時の処理
    $query  = "SELECT id, created_at, namae1,namae2,eiga,zikan,kanin FROM form ORDER BY id ASC";
    $stmt   = $mysqli->prepare($query);

    if (!empty($wheres)) {
        $where  = implode(" AND ", $wheres);
        $query  = "SELECT id, created_at, namae1,namae2,eiga,zikan,kanin FROM form WHERE {$where} ORDER BY id ASC";
        $stmt   = $mysqli->prepare($query);
    }
    try {
        $stmt->execute();
    ...
    <div class="container">
        ...
        <form method="post" action="./list.php">
            <div class="mb-3 row">
                ...
                <div class="col-sm-10">
                    <input type="time" name="zikan" class="form-control" id="zikan" value="<?php if(!empty($_POST['zikan'])) echo $_POST['zikan']; ?>">
                </div>
            </div>
            <div class="mb-3 row">
                ...
                <tbody>
                    <?php foreach ($rows as $row) { ?>
                        <tr>
                            ...
                            <td>
                                <?php echo htmlspecialchars("{$row['namae1']} {$row['namae1']}"); ?>
                                <br>
                                <?php echo htmlspecialchars("{$row['namae2']} {$row['namae2']}"); ?>
                            </td>
                            <td><?php echo htmlspecialchars($row['eiga']); ?></td>
                            <td><?php echo htmlspecialchars($row['zikan']); ?></td>
